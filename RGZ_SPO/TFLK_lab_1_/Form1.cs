...
namespace TFLK_lab_1_
{
    public partial class Form1 : Form
    {
        ...
        private void ПускToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            SyntaxErrorListenerLexer listenerForLexer = new SyntaxErrorListenerLexer();
            string input = box.Text;

            ICharStream stream = CharStreams.fromString(input);
            READLexer lexer = new READLexer(stream);
            lexer.RemoveErrorListeners();
            lexer.AddErrorListener(listenerForLexer);
            ...
            var list = listener.SyntaxErrors;
            var listL = listenerForLexer.SyntaxErrors;

            if(list.Count == 0 && listL.Count == 0)
                this.textResult.Text += "Success";
            else
                this.textResult.Text += "Error\n";
            
            for(int i = 0; i < listL.Count; i++)
            {
                this.textResult.Text += (i+1).ToString() + ") " + listL[i].ToString() + "\n";
            }

            for (int i = 0; i < list.Count; i++)
            {
                this.textResult.Text += (listL.Count + i + 1).ToString() + ") " + list[i].ToString() + "\n";
            }
        }
